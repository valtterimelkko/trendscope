...
const Scene1LogoReveal: React.FC<{ logoUrl: string }> = ({ logoUrl }) => {
  ...
  // Fade out at end of scene (frames 100-120 of the 120-frame scene)
...
const Scene2Montage: React.FC<{ images: string[] }> = ({ images }) => {
  const frame = useCurrentFrame();
  const localFrame = frame - 120; // Adjust for sequence start
  ...
  // Scene timing: Each scene displays for 90 frames (3 seconds) with 30-frame transitions
    ...
    >
      {montageData.map((scene, index) => {
        ...
        if (isActive) {
          ...
          if (sceneProgress < 15) {
            opacity = interpolate(sceneProgress, [0, 15], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
            translateX = interpolate(sceneProgress, [0, 15], [index % 2 === 0 ? 1920 : -1920, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.cubic) });
            ...
            opacity = interpolate(transitionProgress, [0, 15], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
            translateX = interpolate(transitionProgress, [0, 30], [0, index % 2 === 0 ? -1920 : 1920], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.in(Easing.cubic) });
          } else {
            opacity = 1;
            translateX = 0;
          }
          ...
          const transitionProgress = sceneProgress - (sceneDuration - transitionDuration);
          opacity = interpolate(transitionProgress, [0, 20], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
          translateX = interpolate(transitionProgress, [0, 30], [index % 2 === 0 ? 1920 : -1920, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.cubic) });
        } else if (hasPassed) {
          opacity = 0;
        }
        ...
        // Text animations
        const textDelay = 15;
        const textFadeOutStart = sceneDuration - 30;
        const textFadeOutEnd = sceneDuration - 15;
...
// Main Composition - Version 9 (Option A Implementation)
// Enhanced with broadcast-quality effects while keeping V8 timing
export const BloombergIntroV9: React.FC<BloombergIntroProps> = ({
  ...
  return (
    <>
      {/* Scene 1: Enhanced Logo Reveal (0:00 - 0:04 | Frames 0-120) */}
      <Sequence from={0} durationInFrames={120}>
        <Scene1LogoReveal logoUrl={logoUrl} />
      </Sequence>

      {/* Scene 2: Sliding Tile Montage (0:04 - 0:22 | Frames 120-600) */}
      {/* 4 scenes x 90 frames + 3 transitions x 30 frames = 450 frames */}
      <Sequence from={120} durationInFrames={480}>
        <Scene2Montage images={montageImages} />
      </Sequence>

      {/* Scene 3: Title Reveal with Wireframe Globe (0:22 - 0:30 | Frames 600-780) */}
