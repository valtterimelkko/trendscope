...
// Scene 1: Logo Reveal (0:00 - 0:04 | Frames 0-120)
// Extended from 90 to 120 frames for better logo visibility
const Scene1LogoReveal: React.FC<{ logoUrl: string }> = ({ logoUrl }) => {
  ...
  // Logo fade in and scale - larger and more prominent
  ...
  const logoScale = interpolate(
    frame,
    [30, 70],
    [0.7, 1.15],  // Larger final scale (was 0.9 to 1)
  ...
  return (
    ...
    >
      ...
      {/* Logo - Now larger and with transparent background */}
      <div
        style={{
          position: 'absolute',
          opacity: logoOpacity,
      ...
      >
        <Img
          ...
          style={{
            width: 650,  // Larger logo (was 500)
            height: 'auto',
            filter: 'drop-shadow(0 0 60px rgba(0,217,255,0.4)) drop-shadow(0 0 120px rgba(0,102,255,0.2))',
          }}
        />
      </div>
...
// Scene 2: Montage (0:04 - 0:14 | Frames 120-420)
// Extended from 240 to 300 frames - longer text display time
const Scene2Montage: React.FC<{ images: string[] }> = ({ images }) => {
  const frame = useCurrentFrame();
  const localFrame = frame - 120; // Adjust for sequence start

  // Updated marketing-aligned text
  const montageData = [
    { 
      text: 'DETECT EARLY', 
  ...
  // Each scene now lasts 75 frames (2.5s) instead of 60 (2s)
  // Faster transitions: 10 frames (0.33s) instead of 15 (0.5s)
  const sceneIndex = Math.min(Math.floor(localFrame / 75), 3);
  const sceneProgress = localFrame % 75;
  ...
  return (
    ...
    >
      {montageData.map((scene, index) => {
        ...
        if (isActive) {
          // Fast transition in (0-10 frames), hold (10-65), fade out (65-75)
          opacity = interpolate(
            sceneProgress,
            [0, 8, 65, 75],
            [0, 1, 1, 0],
...
// Scene 3: Title Reveal (0:14 - 0:18 | Frames 420-540)
const Scene3TitleReveal: React.FC<{ globeUrl: string }> = ({ globeUrl }) => {
  const frame = useCurrentFrame();
  const localFrame = frame - 420; // Adjust for sequence start
...
// Main Composition
export const BloombergIntroV2: React.FC<BloombergIntroProps> = ({
  logoUrl = 'assets/scene1/trendscope-logo-transparent.png',  // Use transparent logo
  montageImages = [
